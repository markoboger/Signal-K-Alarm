...
class TriggerParserSpec extends AnyWordSpec with Matchers{
  ...
  "A Time Clause 'timeClause' " should {
    "accept expressions of the form 'time is 19:59" in {
      parser.parse(parser.timeCondition, "time is 19:50").get.check(Hours(19)+Minutes(50)) should be(true)
    }
    "not accept expressions with negative time" in {
      Try(parser.parse(parser.timeCondition, "time is -19:59")).isFailure should be(true)
    }
  }

  "A Timer Clause 'timerClause' " should {
    "accept expressions of the form 'timer is 19:59' " in {
      parser.parse(parser.timerCondition, "timer is 19:50").get.check(Hours(19)+Minutes(50)) should be(true)
    }
    "accept negative times of the form 'timer is -05:00' " in {
      parser.parse(parser.timerCondition, "timer is -05:00").get.check(-1*Hours(5)) should be(true)
      parser.parse(parser.timerCondition, "timer is - 0:05").get.check(-1*Minutes(5)) should be(true)
    }
  }

  "A Depth Clause ‘depthClause' " should {
    "accept expressions of the form 'value of Depth is below 5m' " in {
      parser.parse(parser.depthCondition, "value of Depth is below 5m").get.check(Meters(10)) should be(false)
      parser.parse(parser.depthCondition, "value of Depth is below 5m").get.check(Meters(2)) should be(true)
      parser.parse(parser.depthCondition, "value of Depth is above 100 m").get.check(Meters(102)) should be(true)
      parser.parse(parser.depthCondition, "value of Depth is below 20 ft").get.check(Feet(2)) should be(true)
      parser.parse(parser.depthCondition, "value of Depth is between 2m and 5 m").get.check(Meters(3)) should be(true)
      parser.parse(parser.depthCondition, "value of Depth is between 2m and 5 m").get.check(Meters(10)) should be(false)
      parser.parse(parser.depthCondition, "value of Depth is outside 2m and 5 m").get.check(Meters(3)) should be(false)
      parser.parse(parser.depthCondition, "value of Depth is outside 2m and 5 m").get.check(Meters(10)) should be(true)
    }
  }

  "A Distance Clause ‘distanceClause' " should {
    "accept expressions of the form 'distance to Marker is above 5 nm' " in {
      parser.parse(parser.distanceCondition, "distance to Marker is above 500 m").successful should be(true)
      parser.parse(parser.distanceCondition, "distance to Waypoint is below 120 m").successful should be(true)
      parser.parse(parser.distanceCondition, "distance to Photo is between 900 m and 2500 m").successful should be(true)
      parser.parse(parser.distanceCondition, "distance to Marker is above 5 km").successful should be(true)
      parser.parse(parser.distanceCondition, "distance to Waypoint is below 1.20 nm").successful should be(true)
      parser.parse(parser.distanceCondition, "distance to Photo is between 0.9 km and 2.5 km").successful should be(true)
    }
  }
  "A Speed Clause 'speedClause' " should {
    "accept expressions of the form 'value of SOG is above 8 kn' " in {
      parser.parse(parser.speedCondition, "value of SOG is above 8 kn").get.check(Knots(10)) should be(true)
      parser.parse(parser.speedCondition, "value of SOG is above 8 kn").get.check(Knots(6)) should be(false)
      parser.parse(parser.speedCondition, "value of SOG is above 8 kn").get.check(KilometersPerHour(20)) should be(true)
    }
  }

  "A Temperature Clause 'tempClause' " should {
    "accept expressions of the form 'value of Air is above 28°C'" in {
      parser.parse(parser.tempCondition, "value of Air is above 28°C").get.check(Celsius(30)) should be(true)
    }
  }

  "An Angle Clause ‘angleClause' " should {
    "accept expressions of the form 'value of COG is below 100°' " in {
      parser.parse(parser.angleCondition, "value of COG is below 100°").get.check(Degrees(90)) should be(true)
      parser.parse(parser.angleCondition, "value of COG is below 100°").get.check(Degrees(110)) should be(false)
      parser.parse(parser.angleCondition, "value of COG is above 100°").get.check(Degrees(110)) should be(true)
      parser.parse(parser.angleCondition, "value of COG is above 100°").get.check(Degrees(90)) should be(false)
      parser.parse(parser.angleCondition, "value of COG is above 360°").get.check(Degrees(370)) should be(true)
      //parser.parse(parser.angleClause, "value of COG is above 360°").get.check(Degrees(10)) should be(true)
      parser.parse(parser.angleCondition, "value of COG is between 0° and 90°").get.check(Degrees(45)) should be(true)
      parser.parse(parser.angleCondition, "value of COG is between 0° and 90°").get.check(Degrees(100)) should be(false)
      parser.parse(parser.angleCondition, "value of COG is outside 0° and 90°").get.check(Degrees(45)) should be(false)
      parser.parse(parser.angleCondition, "value of COG is outside 0° and 90°").get.check(Degrees(100)) should be(true)
    }
  }
  "A Percent Clause 'percentClause' " should {
    "accept expressions of the form ' value of Battery is below 10%" in {
      parser.parse(parser.percentCondition, "value of Battery is below 10%").get.check(Percent(9)) should be(true)
    }
  }
  "A Trigger" should {
