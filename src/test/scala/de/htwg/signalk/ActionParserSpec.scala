...
class ActionParserSpec extends AnyWordSpec with Matchers{
  ...
  "A Sound Expression 'soundExp' " should {
    "accept expressions of the form 'once'" in {
      parser.parse(parser.soundExp, "once").get should be(SoundRepeat(1))
      parser.parse(parser.soundExp, "twice").get should be(SoundRepeat(2))
    }
    "accept expressions of the form 'three times'" in {
      parser.parse(parser.soundExp, "three times").get should be(SoundRepeat(3))
      parser.parse(parser.soundExp, "eight times").get should be(SoundRepeat(8))
    }
    "accept expressions of the form '3 times'" in {
      parser.parse(parser.soundExp, "6 times").get should be(SoundRepeat(6))
      parser.parse(parser.soundExp, "24 times").get should be(SoundRepeat(24))
    }
    "accept expressions of the form 'until checked'" in {
      parser.parse(parser.soundExp, "until checked").get should be(SoundUntilChecked())
    }
    "accept expressions of the form 'max 23 min'" in {
      parser.parse(parser.soundExp, "max 23 min").get should be(SoundMax(Minutes(23)))
      parser.parse(parser.soundExp, "max 2 seconds").get should be(SoundMax(Seconds(2)))
    }
  }
